...
class Convolution(ZKModel):
    ...
    def __init__(self, file_name = "quantized_conv.pth", rescale = False):
        '''
        #######################################################################################################
        ...
        model = Conv2DModel(dim_1, out_channels, kernel_size=self.kernel_shape, stride=self.strides, padding=self.pads, bias = self.bias).to(device)
        if os.path.exists(file_name):
            checkpoint = torch.load(file_name, map_location=device)
            model.load_state_dict(checkpoint["model_state_dict"])
        model.eval()
        self.model = model
        rescale_config = {"conv": rescale}
...
class QuantizedConvRelu(Convolution):
    def __init__(self, file_name = "quantized_conv.pth", rescale = True):
        ...
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = Conv2DModelReLU(dim_1, out_channels, kernel_size=self.kernel_shape, stride=self.strides, padding=self.pads, bias = self.bias).to(device)
        if os.path.exists(file_name):
            checkpoint = torch.load(file_name, map_location=device)
            model.load_state_dict(checkpoint["model_state_dict"])
        model.eval()
