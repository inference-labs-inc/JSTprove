...
def test_witness_prove_verify_true_inputs_dev(
    ...
    # Unexpected output
    assert (
        stdout.count("poly.num_vars() == *params") == 0
    ), f"'poly.num_vars() == *params' thrown. May need a dummy variable(s) to get rid of error. Dummy variables should be private variables. Can set = 1 in read_inputs and assert == 1 at end of circuit"
    ...
    # Expected output
    assert (
        stdout.count("Running cargo command:") == 3
    ), f"Expected 'Running cargo command: ' in stdout three times, but it was not found."
    assert (
        stdout.count("Witness Generated") == 1
    ), f"Expected 'Witness Generated' in stdout three times, but it was not found."

    assert (
        stdout.count("proving") == 1
    ), f"Expected 'proving' in stdout three times, but it was not found."
    assert (
        stdout.count("Proved") == 1
    ), f"Expected 'Proved' in stdout three times, but it was not found."

    assert (
        stdout.count("Verified") == 1
    ), f"Expected 'Verified' in stdout three times, but it was not found."
...
def test_witness_prove_verify_true_inputs_dev_expander_call(
    ...
    ), f"Expected 'Running cargo command: ' in stdout once, but it was not found."
    assert (
        stdout.count("Witness Generated") == 1
    ), f"Expected 'Witness Generated' in stdout three times, but it was not found."
    ...
    ), f"Expected 'verifying proof' but it was not found."
    assert stdout.count("success") == 1, f"Expected 'success'  but it was not found."

    assert (
        stdout.count("expander-exec verify succeeded") == 1
    ), f"Expected 'expander-exec verify succeeded'  but it was not found."
    assert (
        stdout.count("expander-exec prove succeeded") == 1
    ), f"Expected 'expander-exec prove succeeded'  but it was not found."
...
def test_witness_incorrect_input_shape(
    ...
    for key in input_data:
        ...
        assert (
            torch.as_tensor(input_data[key]).dim() <= 1
        ), f"Input data for {key} is not 1D tensor. This is a testing error, not a model error. Please fix this test to properly flatten."
    ...
    assert os.path.exists(temp_witness_file), "Witness file not generated"
    assert (
        stdout.count("Running cargo command:") == 2
    ), f"Expected 'Running cargo command: ' in stdout twice, but it was not found."

    # Check good output appeared
    for output in GOOD_OUTPUT:
        assert (
            stdout.count(output) == 2
...
def test_witness_unscaled(
    ...
    assert contains_float(
        input_data
    ), f"This is a testing error, not a model error. Please fix this test to properly turn data to float."
    ...
    assert_very_close(
        json.loads(new_output_file), json.loads(written_output_data), model_write
    )
...
def test_witness_unscaled_and_incorrect_and_bad_named_input(
    ...
    assert (
        "input" not in new_input_data.keys()
    ), f"This is a testing error, not a model error. Please fix this test to not incldue 'input' as a key in the input data."
...
def test_witness_wrong_name(
    ...
    assert (
        "output" in new_input_data.keys() or count == 0
    ), f"This is a testing error, not a model error. Please fix this test to incldue 'output' as a key in the input data."
