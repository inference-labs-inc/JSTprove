...
@pytest.mark.parametrize("attrs, match", [
    ...
    # auto_pad modes other than NOTSET are unsupported
    ({"auto_pad": "SAME_UPPER"}, "auto_pad"),
    ({"auto_pad": "VALID"}, "auto_pad"),
    
    # ceil_mode should be 0
    ({"ceil_mode": 1}, "ceil_mode"),
])
def test_maxpool_invalid_params(attrs, match):
    """Ensure InvalidParamError is raised for malformed or unsupported MaxPool attributes."""
    default_attrs = {
        "kernel_shape": [2, 2],
        "pads": [0, 0, 0, 0],
        "strides": [2, 2],
...
@pytest.mark.unit
def test_maxpool_optional_indices_output_unsupported():
    """MaxPool with 2 outputs (value, indices) should raise an error."""
    node = helper.make_node(
        "MaxPool",
        inputs=["input"],
        outputs=["output", "indices"],
        name="maxpool_with_indices",
        kernel_shape=[2, 2],
        pads=[0, 0, 0, 0],
        strides=[2, 2],
        dilations=[1, 1],
    ...
    input_tensor = helper.make_tensor_value_info("input", TensorProto.FLOAT, [1, 3, 32, 32])
    output_tensor = helper.make_tensor_value_info("output", TensorProto.FLOAT, [1, 3, 16, 16])
    indices_tensor = helper.make_tensor_value_info("indices", TensorProto.INT64, [1, 3, 16, 16])

    graph = helper.make_graph(
        nodes=[node],
        name="maxpool_with_indices_graph",
        inputs=[input_tensor],
        outputs=[output_tensor, indices_tensor],
    )
    ...
    with pytest.raises(InvalidParamError, match="optional output|indices"):
        quantizer.check_layer(node, initializer_map)
