import pytest
from python.testing.core.utils.onnx_quantizer.exceptions import QuantizationError, InvalidParamError, UnsupportedOpError, REPORTING_URL

@pytest.mark.unit
def test_quantization_error_message():
    custom_msg = "Something went wrong."
    with pytest.raises(QuantizationError) as exc_info:
        raise QuantizationError(custom_msg)
    assert "The model submitted is not currently supported by JSTProve" in str(exc_info.value)
    assert custom_msg in str(exc_info.value)

    assert REPORTING_URL in str(exc_info.value)

    assert "To enter your model in our support integration queue, please request support for your model in" in str(exc_info.value)

@pytest.mark.unit
def test_invalid_param_error_basic():
...
@pytest.mark.unit
def test_invalid_param_error_with_attr_and_expected():
    with pytest.raises(InvalidParamError) as exc_info:
        raise InvalidParamError(
            node_name="MaxPool_3",
            op_type="MaxPool",
...
@pytest.mark.unit
def test_unsupported_op_error_with_node():
...
@pytest.mark.unit
def test_unsupported_op_error_without_node():
    with pytest.raises(UnsupportedOpError) as exc_info:
