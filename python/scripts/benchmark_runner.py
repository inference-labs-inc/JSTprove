...
# --- metrics parsing (matches your CLI/Rust prints) ---
TIME_PATTERNS = [
...
def parse_metrics(text: str):
    ...
    for pat in MEM_PATTERNS:
        ...
        if m:
            try:
                mem_mb = float(m.group(1)); break
            except ValueError:
                pass
    return time_s, mem_mb

def now_utc():
    # Plain ISO8601 Z for easy parsing downstream
    return datetime.utcnow().isoformat(timespec="seconds") + "Z"

# --- CLI runner ---
def run_cli(phase:str, *, model_path=None, circuit_path=None, quantized_path=None,
            input_path=None, output_path=None, witness_path=None, proof_path=None):
    """
    Run the exact CLI you use by hand, capture stdout/stderr, return rc + metrics.
    """
    base = [sys.executable, "-m", "python.frontend.cli", "--no-banner"]
    if phase == "compile":
        cmd = base + [
            "compile",
            "-m", model_path,
    ...
    # Important: inherit your environment (same as manual runs)
    proc = subprocess.run(
        cmd, capture_output=True, text=True, env=os.environ.copy()
    )
    out = (proc.stdout or "") + (("\n[stderr]\n" + proc.stderr) if proc.stderr else "")
    time_s, mem_mb = parse_metrics(out)
    return proc.returncode, out, time_s, mem_mb, cmd

def summarize(rows, model_name):
    by_phase = {"compile":[], "witness":[], "prove":[], "verify":[]}
    for r in rows:
        if r["model"] == model_name and r["return_code"] == 0 and r["time_s"] is not None:
            by_phase[r["phase"]].append(r["time_s"])
    print(f"\nSummary for {model_name}:")
    for ph in ("compile","witness","prove","verify"):
        vals = by_phase[ph]
        if len(vals) == 0:
            print(f"  {ph:8s}: n=0")
        elif len(vals) == 1:
            print(f"  {ph:8s}: {vals[0]:.3f}s (n=1)")
        else:
            print(f"  {ph:8s}: mean={mean(vals):.3f}s  stdev={stdev(vals):.3f}s  n={len(vals)}")

def main():
    ap = argparse.ArgumentParser(description="Benchmark JSTProve by calling the CLI directly.")
    ap.add_argument("--model", required=True, help="Path to ONNX model (e.g., python/models/models_onnx/lenet.onnx)")
    ap.add_argument("--input", required=False, help="Path to input JSON (if omitted, use your usual one).")
    ap.add_argument("--iterations", type=int, default=5, help="Number of E2E loops (default: 5).")
    ap.add_argument("--output", default="results.jsonl", help="JSONL file to append per-run rows (default: results.jsonl)")
    ap.add_argument("--summarize", action="store_true", help="Print simple per-phase summary at the end.")
    args = ap.parse_args()

    model_path = os.path.abspath(args.model)
    fixed_input = os.path.abspath(args.input) if args.input else None
    ...
    try:
        for it in range(1, args.iterations + 1):
            with tempfile.TemporaryDirectory() as tmp:
                circuit   = os.path.join(tmp, "circuit.txt")
                quantized = os.path.join(tmp, "quantized.onnx")
                output    = os.path.join(tmp, "output.json")
                witness   = os.path.join(tmp, "witness.bin")
                proof     = os.path.join(tmp, "proof.bin")
                inp       = fixed_input or os.path.join(tmp, "input.json")

                for phase in ("compile","witness","prove","verify"):
                    ts = now_utc()
                    rc, log, t, m, cmd = run_cli(
                        phase,
                    ...
                    # surface failures but keep file output
                    if rc != 0:
                        print(f"[{phase}] rc={rc} — see logs below\n{log}\n", file=sys.stderr)
                        # If you want to abort on first failure, uncomment:
                        # raise SystemExit(1)

                    # optional: print quick per-phase line
                    if t is not None:
                        print(f"[{phase}] t={t:.3f}s, mem={m if m is not None else 'NA'} MB")
                    else:
                        print(f"[{phase}] metrics not parsed; rc={rc}")

        print(f"\n✔ Wrote {len(rows)} rows to {out_path}")
        if args.summarize:
            summarize(rows, model_path)
