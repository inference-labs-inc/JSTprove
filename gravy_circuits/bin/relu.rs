...
mod io_reader {
    ...
    */
    ...
    pub(crate) fn input_data_from_json<C: Config>(file_path: &str, mut assignment: ReLUCircuit<<C as Config>::CircuitField>) -> ReLUCircuit<<C as expander_compiler::frontend::Config>::CircuitField>
    {
    ...
    pub(crate) fn output_data_from_json<C: Config>(file_path: &str, mut assignment: ReLUCircuit<<C as Config>::CircuitField>) -> ReLUCircuit<<C as expander_compiler::frontend::Config>::CircuitField>
    {
        // Read the JSON file into a string
        let mut file = std::fs::File::open(file_path).expect("Unable to open file");
        let mut contents = String::new();
