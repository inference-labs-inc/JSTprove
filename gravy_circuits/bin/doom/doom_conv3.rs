...
declare_circuit!(DoomCircuit {
    input_arr: [[[[Variable; 14]; 14]; 32]; 1], // shape (m, n)
    outputs: [[[[Variable; 7]; 7]; 32]; 1], // shape (m, k) 1, 16, 28, 28
});
...
impl<C: Config> Define<C> for DoomCircuit<Variable> {
    fn define<Builder: RootAPI<C>>(&self, api: &mut Builder) {
        ...
        let weights = read_4d_weights(api, &WEIGHTS_INPUT.conv_weights[2]);
        ...
        let out = conv_4d_run(api, input_arr, weights, bias,&WEIGHTS_INPUT.conv_dilation[2], &WEIGHTS_INPUT.conv_kernel_shape[2], &WEIGHTS_INPUT.conv_pads[2], &WEIGHTS_INPUT.conv_strides[2],&WEIGHTS_INPUT.conv_input_shape[2], WEIGHTS_INPUT.scaling, &WEIGHTS_INPUT.conv_group[2], true, v_plus_one, two_v, alpha_2_v, true);
