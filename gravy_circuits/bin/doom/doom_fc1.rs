...
declare_circuit!(DoomCircuit {
    input_arr: [[[[Variable; 7]; 7]; 32]; 1], // shape (m, n)
    outputs: [[Variable; 256]; 1], // shape (m, k) 1, 16, 28, 28
});
...
impl<C: Config> Define<C> for DoomCircuit<Variable> {
    fn define<Builder: RootAPI<C>>(&self, api: &mut Builder) {
        ...
        let out_1d: Vec<Variable> = input_arr.iter()
                .flat_map(|x| x.iter())
                .flat_map(|x| x.iter())
                .flat_map(|x| x.iter())
                .copied()
        ...
        let weights = read_2d_weights(api, &WEIGHTS_INPUT.fc_weights[0]);
        let bias = read_2d_weights(api, &WEIGHTS_INPUT.fc_bias[0]);
