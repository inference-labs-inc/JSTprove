...
template RewardOpt(n, b){
    ...
    signal RATE_OF_DECAY <== 40000000;
    signal RATE_OF_RECOVERY <== 10000000;
    signal FLATTENING_COEFFICIENT <== 90000000;
    signal PROOF_SIZE_THRESHOLD <== 364800000000;
    signal PROOF_SIZE_WEIGHT <== 0;
    signal RESPONSE_TIME_WEIGHT <== 100000000;
    signal MAXIMUM_RESPONSE_TIME_DECIMAL <== 99000000;
    

    for (i=0; i<n; i++) {
        // Confirm verified is a boolean;
        ...
        rate_of_change[i] <== 40000000 - 30000000*verified[i];



        // PlACEHOLDER
        // response_time_normalized
        // Still must include positive check in beginning
        // can remove lower half of clamp when we do this
        ...
        // // PLACEHOLDER 
        // // Implementation can probably be optimized
        // // SOMETHING STILL WRONG HERE
        // proof_size_reward_metric_fn[i] = ProofSizeRewardMetric(b);
        // proof_size_reward_metric_fn[i].PROOF_SIZE_WEIGHT <== PROOF_SIZE_WEIGHT;
        // proof_size_reward_metric_fn[i].proof_size <== proof_size[i];
        // proof_size_reward_metric_fn[i].PROOF_SIZE_THRESHOLD <== PROOF_SIZE_THRESHOLD;
        // proof_size_reward_metric_fn[i].scaling <== scaling;
        // proof_size_reward_metric[i] <== proof_size_reward_metric_fn[i].proof_size_reward_metric;
        // new_score[i] <== proof_size_reward_metric[i];
        proof_size_reward_metric[i] <== 0;

        // calculated_score_fraction
        ...
        // new_score[i] <== isPositive[i];

        
        // Multiplication needs to be scaled back (divide by scaling)
...
component main = RewardOpt(1024,35);