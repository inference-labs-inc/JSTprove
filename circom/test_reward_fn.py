...
class RewardTests():
    ...
    def test_multiplier_ints(self, input_folder:str, proof_folder: str, temp_folder: str, circuit_folder:str):
        """Calculates the torch function (or part of the reward function) that we are trying to circuitize. Run the proof and compare the results
        """
        name = "multiplierTensor"
        ...
        #Function to test
        reward_fn_response = torch.mul(self.response_time.int(), self.maximum_response_time.int()).tolist()
        
        #Put inputs into input file for proof
        inputs = {
            'a': self.response_time.int().tolist(),
            'b': self.maximum_response_time.int().tolist(),
            }
        ...
        for i in range(len(output)):
            # Instead, can make it:
            # assert d[i] - reward_fn_response[i] < 0.00000001
            assert output[i] == reward_fn_response[i]
        print("Outputs match!")
    ...
    def approximate_polynomial(self, x: torch.Tensor) -> torch.tensor:
        ...
        # new_x =  (1/242000*(20*x - 10)**3) + ((20/121)*x) + 0.417355371900826
        # new_x = ((20/121)*x) + 0.417355371900826
        # new_x = 0.417355371900826
        print(new_x)
        return new_x
    
    def test_reward_function(self, input_folder:str, proof_folder: str, temp_folder: str, circuit_folder:str):
        name = "reward"
        witness_file, input_file, proof_path, public_path, verification_key, circuit_name = self._get_files(input_folder, proof_folder, temp_folder, circuit_folder, name)
    ...
    def test_reward_function_optimizations(self, input_folder:str, proof_folder: str, temp_folder: str, circuit_folder:str):
        name = "reward_opt"
        witness_file, input_file, proof_path, public_path, verification_key, circuit_name = self._get_files(input_folder, proof_folder, temp_folder, circuit_folder, name)
...
if __name__ == "__main__":
    ...
    temp_folder = "temp"
    input_folder = "inputs"
    circuit_folder = "circom"
    inputs = generate_sample_inputs()
    reward = RewardTests(**inputs)
    # reward.test_multiplier_ints(input_folder,proof_folder, temp_folder, circuit_folder)
    reward.test_reward_function(input_folder,proof_folder, temp_folder, circuit_folder)
    # reward.test_reward_function_optimizations(input_folder,proof_folder, temp_folder, circuit_folder)
