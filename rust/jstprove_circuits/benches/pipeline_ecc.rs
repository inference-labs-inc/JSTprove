...
use jstprove_circuits::onnx::{
    compile_bn254, compile_goldilocks, compile_goldilocks_basefold, compile_m31, prove_bn254,
    prove_goldilocks, prove_goldilocks_basefold, prove_m31, verify_bn254, verify_goldilocks,
    verify_goldilocks_basefold, verify_m31, witness_bn254_from_f64,
    witness_goldilocks_basefold_from_f64, witness_goldilocks_from_f64, witness_m31_from_f64,
};
use jstprove_circuits::runner::main_runner::read_circuit_msgpack;
use jstprove_onnx::quantizer::{N_BITS_BN254, N_BITS_GOLDILOCKS, N_BITS_M31};
...
fn run_m31_pipeline(model_path: &Path, activations: &[f64]) {
    let metadata =
        expander_metadata::generate_from_onnx_for_field(model_path, N_BITS_M31, None).unwrap();
    let mut params = metadata.circuit_params.clone();
    params.curve = Some(jstprove_circuits::Curve::M31);
    OnnxContext::set_all(
        metadata.architecture,
        metadata.circuit_params,
        Some(metadata.wandb),
    ...
    println!("\n--- M31 Raw (exponent={}) ---", params.scale_exponent);

    let tmp = tempfile::TempDir::new().unwrap();
    let circuit_path = tmp.path().join("circuit_m31.bundle");
    let circuit_path_str = circuit_path.to_str().unwrap();

    let t = Instant::now();
    compile_m31(circuit_path_str, false, Some(params.clone())).unwrap();
    println!("compile: {:>10}", fmt(t.elapsed().as_secs_f64() * 1000.0));
    ...
    let t = Instant::now();
    let proof = prove_m31(&bundle.circuit, &wb.witness, false).unwrap();
    println!("prove:   {:>10}", fmt(t.elapsed().as_secs_f64() * 1000.0));
    println!(
        "proof:   {:>10}",
        format!("{:.1} KiB", proof.len() as f64 / 1024.0)
    ...
    let t = Instant::now();
    assert!(verify_m31(&bundle.circuit, &wb.witness, &proof).unwrap());
    println!("verify:  {:>10}", fmt(t.elapsed().as_secs_f64() * 1000.0));
...
fn main() {
    ...
    run_m31_pipeline(&model_path, &activations);
}
