...
use expander_compiler::frontend::{
    BN254Config, CircuitField, Config, Define, FieldArith, GoldilocksConfig,
    Goldilocksx8OrionConfig, RootAPI, Variable, declare_circuit,
};
...
/// # Errors
/// Returns `RunError` on compilation or serialization failure.
pub fn compile_goldilocks_orion(
    circuit_path: &str,
    compress: bool,
    metadata: Option<CircuitParams>,
) -> Result<(), RunError> {
    crate::runner::main_runner::run_compile_and_serialize::<
        Goldilocksx8OrionConfig,
        Circuit<Variable>,
...
/// # Errors
/// Returns `RunError` on witness generation or activation mismatch.
pub fn witness_goldilocks_orion_from_f64(
    circuit_bytes: &[u8],
    solver_bytes: &[u8],
    params: &CircuitParams,
    activations: &[f64],
    ...
    witness_from_f64::<Goldilocksx8OrionConfig>(
        circuit_bytes,
        solver_bytes,
        params,
        activations,
...
/// # Errors
/// Returns `RunError` on proof generation failure.
pub fn prove_goldilocks_orion(
    circuit_bytes: &[u8],
    witness_bytes: &[u8],
    compress: bool,
) -> Result<Vec<u8>, RunError> {
    prove_from_bytes::<Goldilocksx8OrionConfig>(circuit_bytes, witness_bytes, compress)
}

/// # Errors
/// Returns `RunError` on verification failure.
pub fn verify_goldilocks_orion(
    circuit_bytes: &[u8],
    witness_bytes: &[u8],
    proof_bytes: &[u8],
) -> Result<bool, RunError> {
    verify_from_bytes::<Goldilocksx8OrionConfig>(circuit_bytes, witness_bytes, proof_bytes)
}
