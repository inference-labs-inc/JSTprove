...
pub const COS_HINT_KEY: &str = "jstprove.cos_hint";
...
pub fn compute_cos_quantized(x_q: i64, scale: u64) -> i64 {
    let scale_f64 = scale as f64;
    let x_real = x_q as f64 / scale_f64;
    let y_real = x_real.cos();
...
pub fn cos_hint<F: FieldArith>(inputs: &[F], outputs: &mut [F]) -> Result<(), Error> {
    if inputs.len() != 2 {
        return Err(Error::UserError(format!(
            "cos_hint: expected 2 inputs (x_q, scale), got {}",
            inputs.len()
        )));
    }
    if outputs.len() != 1 {
        return Err(Error::UserError(format!(
            "cos_hint: expected 1 output, got {}",
            outputs.len()
        )));
    }
    ...
    if scale_u256 > U256::from(u64::MAX) {
        return Err(Error::UserError(format!(
            "cos_hint: scale value {scale_u256} exceeds u64::MAX"
        )));
    }
    let scale_u64 = scale_u256.as_u64();
    if scale_u64 == 0 {
        return Err(Error::UserError(
            "cos_hint: scale is zero; cannot de-quantise input".to_string(),
        ));
    }

    let y_q = compute_cos_quantized(x_i64, scale_u64);
...
mod tests {
    ...
    fn run_hint(x_q: i64, scale: u64) -> i64 {
        ...
        cos_hint::<F>(&inputs, &mut outputs).unwrap();
        to_i64(outputs[0])
    }

    #[test]
    fn cos_zero_is_scale() {
        let scale: u64 = 1 << 18;
        assert_eq!(run_hint(0, scale), scale as i64);
    }
    ...
    fn cos_even_symmetry() {
        ...
        assert!(
            (pos - neg).abs() <= 1,
            "even symmetry failed: cos(x)={pos}, cos(-x)={neg}"
        );
    }

    #[test]
    fn cos_bounded_by_scale() {
        let scale: u64 = 1 << 18;
        for &x_real in &[-10.0f64, -1.0, 0.0, 1.0, 10.0] {
            ...
            assert!(
                ...
                "cos output {result} should be in [-scale, scale]"
            );
        }
    }

    #[test]
    fn compute_cos_matches_hint() {
        let scale: u64 = 1 << 18;
        for &x_real in &[-std::f64::consts::PI, -1.0, 0.0, 1.0, std::f64::consts::PI] {
            let x_q = (x_real * scale as f64).round() as i64;
            let from_compute = compute_cos_quantized(x_q, scale);
            let from_hint = run_hint(x_q, scale);
            assert_eq!(
                from_compute, from_hint,
                "compute_cos_quantized and cos_hint disagree for x_real={x_real}"
            );
        }
    }

    #[test]
    fn cos_hint_wrong_input_count() {
        let mut out = [F::zero()];
        assert!(cos_hint::<F>(&[], &mut out).is_err());
        assert!(cos_hint::<F>(&[F::zero()], &mut out).is_err());
    }

    #[test]
    fn cos_hint_zero_scale() {
        let inputs = [F::zero(), F::zero()];
        let mut out = [F::zero()];
        assert!(cos_hint::<F>(&inputs, &mut out).is_err());
    }
}
