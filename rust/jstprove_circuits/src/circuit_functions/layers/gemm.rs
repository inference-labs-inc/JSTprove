...
impl<C: Config, Builder: RootAPI<C>> LayerOp<C, Builder> for GemmLayer {
    fn apply(
        ...
        input_array = check_and_apply_transpose_array(input_array, self.transa, TRANS_A, &LayerKind::Gemm, &self.name)?;
        weights_array = check_and_apply_transpose_array(weights_array, self.transb, TRANS_B, &LayerKind::Gemm, &self.name)?;
        ...
        if self.is_rescale {
            ...
            .map_err(|e| LayerError::Other { layer: LayerKind::Gemm, msg: format!("Rescale failed: {}", e) })?;
        }
...
fn check_alpha_beta(val: f32, var_name: &str, layer_type: LayerKind, layer_name: &str) -> Result<(), LayerError> {
    if val != 1.0 {
