...
impl<C: Config, Builder: RootAPI<C>> LayerOp<C, Builder> for GreaterLayer {
    fn apply(
        &self,
        api: &mut Builder,
        ...
        let out_array =
            ArrayD::from_shape_vec(IxDyn(&self.output_shape), out_flat).map_err(|e| {
                LayerError::InvalidShape {
                    layer: LayerKind::Greater,
                    msg: format!("Greater output reshape failed: {e}"),
                }
            })?;
    ...
    fn build(
        ...
        let a_data: ArrayD<i64> =
            get_w_or_b(layer_context.w_and_b_map, a_name).map_err(|e| LayerError::Other {
                layer: LayerKind::Greater,
                msg: format!("Greater input A '{a_name}' must be a compile-time constant: {e}"),
            })?;

        let b_data: ArrayD<i64> =
            get_w_or_b(layer_context.w_and_b_map, b_name).map_err(|e| LayerError::Other {
                layer: LayerKind::Greater,
                msg: format!("Greater input B '{b_name}' must be a compile-time constant: {e}"),
            })?;
