...
/// # Errors
/// Returns `RunError` on verification or extraction failure.
pub fn verify_and_extract_with_flat<C: Config>(
    circuit: &mut expander_compiler::expander_circuit::Circuit<C::FieldConfig>,
    witness_bytes: &[u8],
    proof_bytes: &[u8],
    num_inputs: usize,
    expected_inputs: Option<&[f64]>,
    ...
    let mpi_config = MPIConfig::verifier_new(1);
    let valid = executor::verify::<C>(circuit, mpi_config, &proof, &claimed_v);
