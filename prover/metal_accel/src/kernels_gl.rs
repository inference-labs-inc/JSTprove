...
use crate::buffers::GOLDILOCKS_ELEM_SIZE;
use crate::device::MetalAccelerator;
use crate::device::THREADGROUP_SIZE;

pub const GOLDILOCKS_R: u64 = 0x185629dcda58878c;
...
pub fn gl_metal_eq_eval_at(
    ...
    gl_build_eq_half_batched(accel, r_first, mul_factor, eq_first_half, cmd_buffer);
    gl_build_eq_half_batched(accel, r_second, &GOLDILOCKS_R, eq_second_half, cmd_buffer);

    {
        let total = 1u64 << r.len();
        let bits_const = write_u32_constant(&accel.device, first_half_bits as u32);
        let pipeline = accel.pipeline("gl_cross_prod_eq");
...
fn gl_build_eq_half_batched(
    ...
    let r_buf = accel.device.new_buffer(
        (r.len() * GOLDILOCKS_ELEM_SIZE) as u64,
    ...
    for i in 0..r.len() {
        ...
        cur = cur
            .checked_shl(1)
            .expect("gl_build_eq_half: cur_eval_num overflow");
    }

    let pipeline = accel.pipeline("gl_build_eq_step");
    cur = 1u32;
    for i in 0..r.len() {
        let encoder = cmd_buffer.new_compute_command_encoder();
        encoder.set_compute_pipeline_state(pipeline);
        encoder.set_buffer(0, Some(eq_buf), 0);
        encoder.set_buffer(1, Some(&r_buf), (i * GOLDILOCKS_ELEM_SIZE) as u64);
...
pub fn gl_metal_vec_add(
    ...
    accel.dispatch_1d("gl_vec_add", count as u64, &[dst, src, &n_const]);
}

pub fn gl_metal_poly_eval(
    accel: &MetalAccelerator,
    ...
    {
        let pipeline = accel.pipeline("gl_poly_eval_kernel");
        let encoder = cmd_buffer.new_compute_command_encoder();
        encoder.set_compute_pipeline_state(pipeline);
    ...
    {
        let reduce_pipeline = accel.pipeline("gl_reduce_blocks");
...
pub fn gl_metal_fold_all(
    ...
    encoder.set_compute_pipeline_state(accel.pipeline("gl_fold_f"));
    ...
    encoder.set_compute_pipeline_state(accel.pipeline("gl_fold_hg"));
