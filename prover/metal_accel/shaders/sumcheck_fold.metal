...
// out[i] = in[2i] + (in[2i+1] - in[2i]) * r
kernel void fold_f(
    device const GoldilocksField* bk_f_in [[buffer(0)]],
    device GoldilocksField* bk_f_out [[buffer(1)]],
    ...
    GoldilocksField f0 = bk_f_in[tid * 2];
    GoldilocksField f1 = bk_f_in[tid * 2 + 1];
...
kernel void fold_hg(
    device const GoldilocksField* bk_hg_in [[buffer(0)]],
    device GoldilocksField* bk_hg_out [[buffer(1)]],
    device const uint32_t* gate_exists_in [[buffer(2)]],
    device uint32_t* gate_exists_out [[buffer(3)]],
    device const GoldilocksField* r_val [[buffer(4)]],
    ...
    GoldilocksField r = r_val[0];
    GoldilocksField h0 = bk_hg_in[tid * 2];
    GoldilocksField h1 = bk_hg_in[tid * 2 + 1];
